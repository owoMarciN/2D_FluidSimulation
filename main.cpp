...
bool Init() {
    ...
    window = SDL_CreateWindow("2D Fluid Simulation", 640, 480, 0);
    if (!window) {
...
void RenderDensity(FluidCell *fluid, SDL_Renderer *mRenderer) {
    for (int j = 0; j < N; j++) {
        for (int i = 0; i < N; i++) {
...
void Start(FluidCell* fluid, SDL_Renderer* renderer) {
    SDL_Event event;
    bool running = true;
    ...
    std::uniform_real_distribution<double> randDensity(50.0, 150.0);
    ...
    while (running) {
        ...
        if (mousePressed && fx >= 0 && fx < N && fy >= 0 && fy < N) {
            // Add density in 3x3 area around the mouse
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
